	...
	else
	{
		if( abs(delta.y()) > abs(delta.x()) )
		{
			...
			else if( mode == IM_45_90 || mode == IM_90_45 )
			{
				...
				if( dy > 0 )
					vert = delta.y() - abs(delta.x());	// positive
				else
					vert = delta.y() + abs(delta.x());	// negative
				if( mode == IM_90_45 )
					p.setY( pi.y + vert );
		...
		else
		{
			...
			else if( mode == IM_45_90 || mode == IM_90_45 )
			{
				...
				if( dx > 0 )
					hor = delta.x() - abs(delta.y());	// positive
				else
					hor = delta.x() + abs(delta.y());	// negative
				if( mode == IM_90_45 )
					p.setX(pi.x + hor);
				else if( mode == IM_45_90 )
				{
...
void RotateRect( QRect &r, int angle, QPoint org )
{
	...
	if( angle == 90 )
	{
		left = org.x() + (r.bottom()+1 - org.y);
		right = org.x() + (r.top() - org.y);
		top = org.y() + (org.x() - r.right()+1);
		bottom = org.y() + (org.x() - r.left());
		if( left > right )
		{
			int temp = right;
			left = right;
...
//
double StrToDimension( QString & str, int def_units, bool bRound10 )
{
	double dim;
	int mult;
...
//
QString DimensionToStr( int dim, int units, bool append_units,
							  bool lower_case, bool space, int max_dp, bool strip )
{
	QString str = "%1";
	if( units == MM )
		str = str.arg((double)dim/1000000.0, 0, 'f', max_dp);
	else if( units == MIL )
		str = str.arg((double)dim/NM_PER_MIL, 0, 'f', max_dp);
	else if( units == NM )
		str = str.arg(dim);
	else
		ASSERT(0);
...
// sets prefix to alphanumeric prefix
//
int ParseRef( QString ref, QString & prefix )
{
	...
	if( num_length ==  0 )
		return 0;
	QString num_str = ref.right( num_length );
...
//
bool CheckLegalPinName( QString pinstr, QString & astr, QString & nstr, int & n )
{
	pinstr = pinstr.trimmed();
	if (pinstr.length() == 0)
		return false;

	QRegExp regex("^([a-zA-Z\\-\\+\\_])*(\\d)*$");
	bool matched = regex.exactMatch(pinstr);
	if (!matched)
		return false;
	astr = regex.cap(1);
	nstr = regex.cap(2);
								 ...
								 double x[], double y[] )
{
	...
	if( style != Polygon::STRAIGHT && style2 != Polygon::STRAIGHT )
	{
	...
	else if( style == Polygon::STRAIGHT )
	{
	...
	else if( style2 == Polygon::STRAIGHT )
	{
								...
								double * dist )
{
	...
	if( xf != xi )
	{
		...
		if( style == Polygon::STRAIGHT || yf == yi )
		{
		...
		else if( style == Polygon::ARC_CW || style == Polygon::ARC_CCW )
		{
			// arc (quadrant of ellipse)
			// convert to clockwise arc
			int xxi, xxf, yyi, yyf;
									   ...
									   int * x, int * y, double * d )
{
	...
	else if( x1i == x1f )
	{
		...
		a = (double)y2i - b*x2i;
		double x1, y1, x2, y2;
		int test = FindLineSegmentIntersection( a, b, x1i, y1i, x1f, y1f, Polygon::STRAIGHT,
			&x1, &y1, &x2, &y2 );
		if( test )
		{
	...
	else if( x2i == x2f )
	{
		...
		a = (double)y1i - b*x1i;
		double x1, y1, x2, y2;
		int test = FindLineSegmentIntersection( a, b, x2i, y2i, x2f, y2f, Polygon::STRAIGHT,
			&x1, &y1, &x2, &y2 );
		if( test )
		{
...
void DrawArc( QPainter * painter, int shape, int xxi, int yyi, int xxf, int yyf )
{
	...
	else if( shape == DL_ARC_CCW || shape == DL_ARC_CW ) 
	{
		...
		painter->drawArc(QRect(QPoint(xc-r, yc-r), QPoint(xc+r, yc+r)), startAngle, -90*16);
								   ...
								   int max_cl, int * x, int * y )
{
	...
	if( style1 == Polygon::STRAIGHT && style1 == Polygon::STRAIGHT )
	{
	...
	else if( style1 == Polygon::STRAIGHT )
	{
